class ProductsController < ApplicationController
  def index
    ...
    if params[:category].present?
      ...
      @products = Product.where(:category => @category.id).published

    # 判斷是否筛选大类
    elsif params[:group].present?
      @group_s = params[:group]
      @group = CategoryGroup.find_by(name: @group_s)
      @products = Product.joins(:category).where("categories.category_group_id" => @group.id).published

    # 判斷是否筛选品牌
    elsif params[:brand].present?
      @brand_s = params[:brand]
      @brand = Brand.find_by(name: @brand_s)
      @products = Product.where(:brand => @brand.id).published
